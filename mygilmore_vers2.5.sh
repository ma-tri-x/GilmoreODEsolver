...
thisdir=$(pwd)
   ...
   Rstart=20e-6 #5.45126e-6
   RdotStart=0.0 #m/s
   Rn=184.1e-6     #m
   Rn2=64e-6
   t_transit_start=50e-6
   t_transit_end=60e-6
   deltaT=1e-11  #s
   Tstart=0.    #s
   Tend=115e-6  #s
...
if [[ ! -e ${scriptpath}/mygilmore_vers2.5 ]]
then 
   echo "g++ -O3 mygilmore_vers2.5.c -o mygilmore_vers2.5"
   g++ -O3 ${scriptpath}/mygilmore_vers2.5.c -o ${scriptpath}/mygilmore_vers2.5
fi

${scriptpath}/mygilmore_vers2.5 $Rstart $RdotStart $Rn $deltaT $Tstart $Tend $patm $pac $f $bVan $mu $sigma $BTait $nTait $kappa $pv $rho0 $SpecGasConst $TempRef $T0 $epsilon $phase $Rn2 $t_transit_start $t_transit_end
